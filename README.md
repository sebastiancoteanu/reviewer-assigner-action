...
- **Release tag** (best for consumers): `@v1.0.3` (immutable patch pin)
- **Branch** (okay for early iteration): `@main`
- **Commit SHA** (most deterministic): `@<40-char-sha>`

Note: a moving major tag like `@v1` may exist, but **immutable pins** (`@v1.0.3` or a full SHA) are safer for production consumer repos.
